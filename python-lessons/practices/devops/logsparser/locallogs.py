...
class LogsIndex:
    ...
    def get_offsets(self, begin_key, end_key):
        ...
        logging.debug('get_index_range:%s %s %s %s %s %s', self.indexes[0], self.indexes[-1], begin_key, end_key, left,
                      right)
...
class LogsDB:
    ...
    def append_data(self, data, record_time=datetime.now()):
        def check_args():
            ...
            if not isinstance(data, itertools.basestring):
...
def test():
    ...
    def time_test(test_name):
        def inner(f):
            def inner2(*args, **kargs):
                ...
                result = f(*args, **kargs)
                print
                '%s take time:%s' % (test_name, (datetime.now() - start_time))
                return result
    ...
    def test_get_data(db):
        ...
        print
        'test_get_data get %s results' % len(results)
